...
* Provided dist files are configured to be served from the root of your domain. If you need to serve shlink-web-client from a sub path, you will have to build it yourself by following [these steps](/documentation/shlink-web-client/serve-in-sub-path).
* The app has a client-side router that handles dynamic paths. Because of that, you need to configure your web server to fall-back to the `index.html` file when requested files do not exist.
    ...
    * If you use nginx, you can [see how it's done](https://github.com/shlinkio/shlink-web-client/blob/master/config/docker/nginx.conf) for the docker image and do the same.
