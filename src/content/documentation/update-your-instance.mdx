...
Env vars will be backwards compatible, and only major releases may drop support for some of the existing ones, but when this happens, documentation will be provided on how to migrate.

### Updating a self-hosted instance.

If you are hosting Shlink manually, whether it is served with swoole/openswoole or not, you probably started by running the `cli/install` helper tool, which gets your instance set-up.

There's an updating counterpart of this tool, `cli/update`. With it, you will be able to import the configuration from your previous installation folder, and provide any new configuration options. Any configuration option you previously provided will be kept the same.
...
4. If you are using shlink with swoole or openswoole, restart the service by running `/etc/init.d/shlink_swoole restart`.
