...
Commands in shlink are contextually namespaced in `api-key`, `db`, `domain`, `short-url`, `tag` and `visit`.
...
Available commands:
 ...
 domain
  domain:list         List all domains that have been ever used for some short URL, or have some "not found" redirect configured.
