...
<Callout type="info">
  ...
  In order to install images from the former, use `ghcr.io/shlink/shlink` instead of `shlink/shlink`.
</Callout>
...
It can be either one server name or a comma-separated list of servers (make sure to include the port number): `REDIS_SERVERS=shlink.abcd.cache.amazonaws.com:6379,another.efgh.cache.amazonaws.com:1234`
...
<Callout type="warning">
  Take into consideration that there are no versions older than 3.0.0 published in GitHub Container registry.
</Callout>
