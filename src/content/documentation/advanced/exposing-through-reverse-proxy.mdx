...
When you are serving Shlink with swoole/openswoole or using the docker image (which uses openswoole internally), you'll probably want to put a reverse proxy in front of it.

Swoole and openswoole have some limitations, that can be solved by using a reverse proxy:
...
It is possible to achieve both just by tweaking swoole/openswoole, but it is recommended to, instead, put a standard HTTP server in front of it, like nginx or apache, as swoole implements only a subset of the HTTP standard.
...
Example nginx configuration to use as a reverse proxy in front of swoole/openswoole, with SSL enabled:
