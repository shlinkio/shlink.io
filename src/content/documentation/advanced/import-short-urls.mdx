...
<Callout type="warning">
  YOURLS' API does not expose enough actions for Shlink to fetch all data. Because of that, you first need to install the <code className="no-highlight indivisible">yourls-shlink-import-plugin</code> in YOURLS itself. You can download it from <Link href="https://slnk.to/yourls-import">its repository</Link> and follow the instructions there.
</Callout>
