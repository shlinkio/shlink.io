...
<Callout type="warning">
  Enabling this feature has some edge cases to consider when [multi-segment slugs](#multi-segment-custom-slugs) are also enabled:

  * If you explicitly create a URL with the slug `whatever/`, Shlink will try to match only `whatever` as the slug, resulting in a 404.
  * If you create both `whatever` and `whatever/`, Shlink will always match the first one.
</Callout>
