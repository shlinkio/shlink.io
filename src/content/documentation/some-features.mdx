...
This is achieved via the so called `loosely` short URL mode (the mode can be set via [installation tool](/documentation/command-line-interface/installation-tool) or [environment variables](/documentation/environment-variables)).

<Callout type="warning">
  Setting `loosely` mode has a couple of considerations:
