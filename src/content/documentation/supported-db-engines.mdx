...
It's **very important** to understand that using SQLite for production is not officially supported, since you can quickly face problems and limitations when dealing with big amounts of data.

Some have even tried to use it together with docker, via docker volumes. That's just looking for problems.

Do yourself a favor and move to one of the other engines.
