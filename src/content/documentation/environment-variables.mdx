...
* Shlink also supports providing configuration options via the so called "install tool" (`vendor/bin/shlink-installer install` script). This is an interactive CLI tool that generates some "hardcoded" configuration after being executed.
