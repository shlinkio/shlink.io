...
<Callout type="info">
  <p>Shlink has included a default GeoLite2 license key since version 1.21.1, but it is currently disabled, and not providing your own will result in some annoying errors being logged.</p>
  Shlink 3.0, when released, will no longer include this license key, and not providing one will, instead, completely disable geolocation.
</Callout>
...
* Via installer tool: you will be asked to provide the license key when installing or updating Shlink, or you can use `bin/set-option` to set it later.
