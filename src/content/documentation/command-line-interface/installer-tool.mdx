...
In order to list all available commands from the installer tool, just run `vendor/bin/shlink-installer` with no arguments:
...
The installer tool is designed to make a couple of main actions: install Shlink, update an existing instance or update the value from a particular config option.
