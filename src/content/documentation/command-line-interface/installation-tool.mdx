...
Options:
      ...
      --update-roadrunner-binary  Try to update the RoadRunner binary. Providing this flag for a non-RoadRunner Shlink instance will result in an error.
...
* `--update-roadrunner-binary`: If you are serving Shlink with RoadRunner, it is important to pass this when updating your instance, so that a new binary is downloaded.
