...
<Callout type="warning" title="Be extremely careful when using this feature">
  ...
  <br />
  Because of that, make sure you use this only when you self-host shlink-web-client, and you know only trusted people
...
- Mount a `conf.d` directory, which in turn contains the `servers.json` file, in a volume inside `/usr/share/nginx/html`. _(since shlink-web-client 3.2.0)_.

  ```shell
  docker run --name shlink-web-client -p 8000:8080 -v ${PWD}/my-config/:/usr/share/nginx/html/conf.d/ shlinkio/shlink-web-client
  ```

- Provide the config for a default server via env vars. When the container starts up, it will build the `servers.json` file dynamically based on them. _(since shlink-web-client 3.2.0)_.
  ...
  - `SHLINK_SERVER_FORWARD_CREDENTIALS` _(v4.4.0)_ : Whether credentials should be forwarded (`true`) or not (`false`). Defaults to `false`.
