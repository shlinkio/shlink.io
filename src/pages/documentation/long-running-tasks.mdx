...
<Callout type="info">
  These tasks are not strictly required when serving Shlink with [RoadRunner](/documentation/supported-runtimes/serve-with-roadrunner/), as it has its own background jobs system that Shlink uses to delegate these tasks.
</Callout>
