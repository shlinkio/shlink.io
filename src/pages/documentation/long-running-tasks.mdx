...
<Callout type="info">
  These tasks are not strictly required when serving Shlink with
  [RoadRunner](/documentation/supported-runtimes/serve-with-roadrunner/), as it has its own background jobs system that
  Shlink uses to delegate these tasks.
</Callout>
...
_The `-q` and `-n` flags are available on any Shlink command. The first one will discard any output, and the second one ensure the command is run in a non-interactive mode._
